	...
	class CountSubMethodTest{
	    ...
	    String show_methodName() throws Exception{
	    	...
	    	for(int i=0;i<methodList.size();i++){
	    		...
	    		//str+=String.format("%-20s%-2s%-5s\n",methodList.get(i),":",countSubMethodList.get(i));
	    		if(i==methodList.size()-1){
	    			...
	    			double value;
	    			 ...
	    			 double vtemp;
	    			 String ntemp;
	    			   for (int m = 0; m < methodList.size()-1; m++) {
                           
                           for (int n = m+1; n < methodList.size(); n++) {
                        	   value = countSubMethodList.get(m);
                        	   name= methodList.get(m);
                        	   
                        	   double temp_value=countSubMethodList.get(n);
                        	   String temp_name= methodList.get(n);   
                        	   if (value<temp_value){
                        		   ntemp=temp_name;
                        		   vtemp=temp_value;
                        		   
                        		   methodList.set(n, name);
                        		   countSubMethodList.set(n, value);
                        		   
                        		   methodList.set(m, ntemp);
                        		   countSubMethodList.set(m, vtemp);
                        		   
                        	   }
                           }
	    			   }
	    			   // 80/20法則，將高低值存入subhighlowList中
	    			   String high="高";
	    			   String low="低";
	    			 for(int k=0;k<methodList.size();k++){
	    				 //str+= temp_index[k]+" "+temp_Arr[k][0]+"\n";
	    				 if(k<=methodList.size()*0.2-1){
	    					 if(countSubMethodList.get(k)==countSubMethodList.get(k+1)){
	    						 subhighlowList.add(low);
		    					 str+=String.format("%-20s%-2s%-11s%-5s\n",methodList.get(k),":",countSubMethodList.get(k),subhighlowList.get(k));
		    				 }
	    					 else{
	    						 subhighlowList.add(high);
	    				 str+=String.format("%-20s%-2s%-11s%-5s\n",methodList.get(k),":",countSubMethodList.get(k),subhighlowList.get(k));
	    					 }
	    				 }
	    			 ...
	    			 /*for(int k=0;k<methodList.size();k++){
	    				 str+=String.format("%-20s%-2s%-5s\n",methodList.get(k),":",countSubMethodList.get(k));
	    			 }*/
